...
$(document).on('ready page:load turbolinks:load', function () {
  $('a.modalOrder').click(function(e){
    ...
    ActiveAdmin.modal_dialog("Selecciona la tarifa", {tarifa: ["Colegio","Curso","ICFES","Univ"]}, function(inputs) {
      $.get("/admin/students/"+id+"/neworden",inputs,function(data){ document.write(data); document.close();});
    })
  })

  $("#order_materias_attributes_0_weekly_input").change(function(){
      if ($(this).is(':checked')){
        $(this).find("#order_materias_attributes_0_until_re_date").show();
      }
      else {
        $(this).find("#order_materias_attributes_0_until_re_date").hide(); 
      }
    });
  }
